...
public abstract class BasePage {
    ...
    public void click(By locator) {
        ...
        try {
            // Step 1: Wait for any loading overlays to disappear
            ...
            // Step 4: Add small delay for scroll animation
            Thread.sleep(300);
            
            // Step 5: Try regular click
            element.click();
            LoggerUtil.info(logger, "Element clicked successfully: " + locator);
            
        } catch (ElementClickInterceptedException e) {
            LoggerUtil.warn(BasePage.class, "Regular click intercepted, attempting JavaScript click: " + locator);
            
            try {
                // Retry: Wait for overlay again and use JavaScript click
                waitForLoadingSpinnerToDisappear();
                WebElement element = WaitUtil.waitForElementToBeClickable(driver, locator);
                ((org.openqa.selenium.JavascriptExecutor) driver)
                        .executeScript("arguments[0].click();", element);
                LoggerUtil.info(logger, "Element clicked using JavaScript: " + locator);
                
            } catch (Exception jsClickError) {
                LoggerUtil.error(logger, "JavaScript click also failed: " + locator, jsClickError);
                throw new RuntimeException("Failed to click element after multiple attempts: " + locator, jsClickError);
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            LoggerUtil.error(logger, "Click operation interrupted: " + locator, e);
            throw new RuntimeException("Click operation was interrupted: " + locator, e);
        }
    }
